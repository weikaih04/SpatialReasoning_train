#!/usr/bin/env python3
"""
Convert leo66666/messytable (train split) to ThinkMorph unified_edit parquet format.

Visual thought only (no text reasoning): input images + reasoning_image_0 as visual thought + answer.
Format matches base MVC (single output image with empty think tags).
...
Output schema per row:
  ...
  - num_input_images: N  (2-7 input views, 1 output reasoning image)
"""
...
def generate_mcq_choices(gt_answer, seed):
    ...
    pool = [x for x in range(max(1, gt_answer - 3), min(8, gt_answer + 3) + 1) if x != gt_answer]
    if len(pool) < 3:
        pool = [x for x in range(1, 9) if x != gt_answer]
...
def reformat_sample(sample, idx):
    ...
    if not images or gt_answer is None or not question or reasoning_img is None:
        return None

    options, correct_letter = generate_mcq_choices(int(gt_answer), seed=42 + idx)

    choices_str = "\n".join(f"({LETTERS[i]}) {v}" for i, v in enumerate(options))
    full_question = f"{question}\n{choices_str}"

    # Input images (2-7 views) + 1 output image (reasoning_image_0)
...
def main():
    ...
    parser.add_argument(
        ...
        default="/weka/oe-training-default/jieyuz2/improve_segments/visual_cot/"
                "ThinkMorph_training/bagel_example/editing/messytable",
    )
    parser.add_argument("--num_shards", type=int, default=5)
    args = parser.parse_args()

    print("Loading leo66666/messytable train split...")
    ds = load_dataset("leo66666/messytable", split="train")
